...
import { TESTIMONIAL_COMPONENTS } from './TestimonialsLibrary';
import { FAQ_COMPONENTS } from './FAQLibrary';
import { LOGO_CLOUD_COMPONENTS } from './LogoCloudLibrary';
...
export const Storefront: React.FC<StorefrontProps> = ({ config, products, pages, activePageId, onNavigate, previewBlock, activeBlockId, onUpdateBlock }) => {
  ...
  const renderBlock = (block: PageBlock) => {
    ...
    switch (block.type) {
      ...
      case 'system-testimonials':
        const TestimonialComponent = TESTIMONIAL_COMPONENTS[block.variant || 'minimal'];
        return <div key={block.id}><TestimonialComponent data={block.data} primaryColor={config.primaryColor} /></div>;
      case 'system-faq':
        const FAQComponent = FAQ_COMPONENTS[block.variant || 'accordion'];
        return <div key={block.id}><FAQComponent data={block.data} primaryColor={config.primaryColor} /></div>;
      case 'system-logo-cloud':
        const LogoCloudComponent = LOGO_CLOUD_COMPONENTS[block.variant || 'simple'];
        return <div key={block.id}><LogoCloudComponent data={block.data} primaryColor={config.primaryColor} /></div>;
      case 'section':
      default:
        return (
          <div
