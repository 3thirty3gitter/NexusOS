...
import { TESTIMONIAL_OPTIONS } from './TestimonialsLibrary';
import { FAQ_OPTIONS } from './FAQLibrary';
import { LOGO_CLOUD_OPTIONS } from './LogoCloudLibrary';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState<'hero' | 'grid' | 'content' | 'social-proof' | 'support' | null>(null);
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          {addSectionStep === 'categories' ? (
            <div className="space-y-2">
              ...
              <button onClick={() => { setSelectedCategory('social-proof'); setAddSectionStep('options'); }} className="w-full p-4 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 hover:border-yellow-500 rounded-xl flex items-center justify-between group transition-all">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-yellow-900/20 text-yellow-500 rounded-lg group-hover:bg-yellow-500 group-hover:text-white transition-colors"><Quote size={24} /></div>
                  <div className="text-left">
                    <span className="block text-sm font-bold text-white">Social Proof</span>
                    <span className="text-xs text-neutral-500">Testimonials & Logos</span>
                  </div>
                </div>
                <ChevronDown className="-rotate-90 text-neutral-600" />
              </button>

              <button onClick={() => { setSelectedCategory('support'); setAddSectionStep('options'); }} className="w-full p-4 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 hover:border-blue-500 rounded-xl flex items-center justify-between group transition-all">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-blue-900/20 text-blue-500 rounded-lg group-hover:bg-blue-500 group-hover:text-white transition-colors"><HelpCircle size={24} /></div>
                  <div className="text-left">
                    <span className="block text-sm font-bold text-white">Support</span>
                    <span className="text-xs text-neutral-500">FAQ & Help</span>
                  </div>
                </div>
                <ChevronDown className="-rotate-90 text-neutral-600" />
              </button>
            ...
            <div className="space-y-4">
              ...
              {selectedCategory === 'social-proof' && (
                <div className="space-y-6">
                  <div>
                    <h4 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-2">Testimonials</h4>
                    <div className="grid grid-cols-1 gap-2">
                      {TESTIMONIAL_OPTIONS.map(opt => (
                        <button key={opt.id} onClick={() => handlePreviewBlock('system-testimonials', `Testimonial: ${opt.name}`, '', opt.id)} className={`text-left p-3 rounded-xl border transition-all ${previewBlock?.variant === opt.id && previewBlock.type === 'system-testimonials' ? 'bg-yellow-600/20 border-yellow-500 text-white' : 'bg-neutral-900 border-neutral-800 text-neutral-400 hover:border-neutral-600'}`}>
                          <div className="font-bold text-sm">{opt.name}</div>
                          <div className="text-[10px] opacity-60">{opt.description}</div>
                        </button>
                      ))}
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-2">Logo Cloud</h4>
                    <div className="grid grid-cols-1 gap-2">
                      {LOGO_CLOUD_OPTIONS.map(opt => (
                        <button key={opt.id} onClick={() => handlePreviewBlock('system-logo-cloud', `Logos: ${opt.name}`, '', opt.id)} className={`text-left p-3 rounded-xl border transition-all ${previewBlock?.variant === opt.id && previewBlock.type === 'system-logo-cloud' ? 'bg-yellow-600/20 border-yellow-500 text-white' : 'bg-neutral-900 border-neutral-800 text-neutral-400 hover:border-neutral-600'}`}>
                          <div className="font-bold text-sm">{opt.name}</div>
                          <div className="text-[10px] opacity-60">{opt.description}</div>
                        </button>
                      ))}
              ...
              {selectedCategory === 'support' && (
                <div className="grid grid-cols-1 gap-2">
                  {FAQ_OPTIONS.map(opt => (
                    <button key={opt.id} onClick={() => handlePreviewBlock('system-faq', `FAQ: ${opt.name}`, '', opt.id)} className={`text-left p-3 rounded-xl border transition-all ${previewBlock?.variant === opt.id ? 'bg-blue-600/20 border-blue-500 text-white' : 'bg-neutral-900 border-neutral-800 text-neutral-400 hover:border-neutral-600'}`}>
                      <div className="font-bold text-sm">{opt.name}</div>
                      <div className="text-[10px] opacity-60">{opt.description}</div>
                    </button>
                  ))}
