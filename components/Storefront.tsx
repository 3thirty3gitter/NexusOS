...
import { SECTION_OPTIONS } from './SectionLibrary';
import { Plus } from 'lucide-react';

export const Storefront: React.FC<StorefrontProps> = ({ config, products, pages, activePageId, onNavigate, previewBlock, activeBlockId, onUpdateBlock, scrolledBlockId }) => {
  const [cart, setCart] = useState<Product[]>([]);

  // Auto-scroll effect
  React.useEffect(() => {
    if (scrolledBlockId) {
      const element = document.getElementById(`block-${scrolledBlockId}`);
      if (element) {
  ...
  const renderBlock = (block: PageBlock) => {
    ...
    switch (block.type) {
      ...
      case 'system-grid':
        return <div key={block.id} id={`block-${block.id}`}>{renderProductGrid(block.variant, block.data)}</div>;
      case 'system-ui':
        // Find the component across all categories
        let UIComponent = null;
        for (const cat in SECTION_OPTIONS) {
          const found = SECTION_OPTIONS[cat].find(opt => opt.id === block.variant);
          if (found) {
            UIComponent = found.component;
            break;
          }
        }
        return UIComponent ? (
          <UIComponent
            ...
            onUpdate={(data) => onUpdateBlock && onUpdateBlock(block.id, data)}
          />
        ) : <div className="p-12 text-center text-red-500">Component not found: {block.variant}</div>;
      case 'section':
      default:
        return (
          <div
